...
public class View {
    ...
    private static final String NO_ITEM_TO_UPDATE = "No item to update";
    ...
    private void displayEnterQuantity(int quantity) {
        ...
        if (updatedItem == null) {
            appendLine(builder, NO_ITEM_TO_UPDATE);
            print(builder);
            return;
        }
    ...
    private void displayChange(double paymentAmount) {
        StringBuilder builder = new StringBuilder();
        appendLine(builder, String.format(PAYMENT, formatPrice(paymentAmount)));
        Amount change = controller.enterPayment(Amount.of(paymentAmount));
        appendLine(builder, String.format(CHANGE, formatAmount(change)));
        endSection(builder);
        print(builder);
    }
